...
##############################################
# 1. Homebrew 업데이트 & 필수 패키지 설치
##############################################
echo "=== [1] Homebrew 업데이트 & 필수 패키지 설치 ==="

# brew 명령어가 없는 경우 설치 안내
if ! command -v brew &> /dev/null; then
  echo "[ERROR] Homebrew가 설치되어 있지 않습니다. 먼저 Homebrew를 설치해 주세요."
  echo "       설치 명령 예) /bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\""
  exit 1
fi
...
if ! command -v gaianet &> /dev/null; then
  ...
  if [ "$CURRENT_SHELL" = "zsh" ] && [ -f "$HOME/.zshrc" ]; then
    ...
    echo "[WARN] bashrc나 zshrc 파일을 찾지 못했거나 다른 셸을 사용 중일 수 있습니다."
  ...
  if ! command -v gaianet &> /dev/null; then
    ...
    echo "[ERROR] 새 터미널을 열거나, ~/.bashrc(또는 ~/.zshrc)를 다시 source 하거나, 심볼릭 링크를 만드세요."
    exit 1
  fi
